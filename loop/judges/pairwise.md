...
Each winner is A, B or tie. Every dimension needs exact quotations from BOTH books and an explanation. When `evaluation_goal` says this is a no-regression comparison, a tie is a successful preservation result: do not manufacture a winner because one book is newer, longer, more cautious, more detailed, or more polished. Prefer one side only when the quoted passages show a meaningful difference on that dimension. Each critical finding is {"kind":"OVERCLAIM","quote":"Exact passage","explanation":"Why material"}; use a finding kind from the shared v2 schema. Empty critical lists mean explicitly checked with none found, not a compulsory PASS. The runtime reverses label order in an independent call and counts the pair as one generation sample. Do not invent source-verification or human-reader evidence you were not given.
