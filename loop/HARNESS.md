...
|---|---|---|---|
| **pi coding agent** | `.pi/agents/*.md` (frontmatter `model:`) | the `subagent` tool | endpoint/auth/route bindings are the `[PI BINDING]` fields in `loop/config.yaml`; Muse Spark roles use the `vercel` provider in `~/.pi/agent/models.json` (Vercel AI Gateway). Preflight runs via `scripts/loop-runner/run_preflight.sh` |
