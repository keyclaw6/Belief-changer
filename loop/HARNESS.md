...
The external profile is intentionally null. Select and configure an independent model family explicitly. There is no silent fall-through to the generator when external capacity fails. Research access follows docs/RESEARCH-ACCESS.md. Browser operations must use the dedicated CloakBrowser/NopeCHA profile and Agent-Reach/OpenCLI social backend with a fresh live preflight. Direct permitted HTTP/RSS/PDF retrieval remains available for primary sources; no stock-browser fallback. Access failure is not scarcity.
