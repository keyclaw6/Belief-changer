...
### iter-050 — chorus discipline + contradiction cleanup (final authorized)
**Hypothesis:** PRIMARY deficit: freeze chorus to mantra echoes (kills variant families) + delete dead :224 parenthetical; carried lines unchanged. Deficit falls both (30→≤22 / 40→≤30).
**Change:** `prompts/style-guide.md` (chorus bullet, vocab parenthetical). Both plans regenerated (sugar 16, smoking 15). Research reused.
**Verdict:** QUANTIFY
**Lesson:** Split outcome on identical edits. Smoking hit band (40→28) via regen-draw variance (drilled-you dropped, no CAPS law in that plan). Sugar crashed (30→58) on pre-existing plan doctrine: every-sentence CAPS law tails/suffixes every sentence (FRAGMENT-blind: complete + belief verb); paraphrase+suffix breaks frozen endings; reviewer ID finding overrides carried silence at runtime (first contradiction downstream of adequate card+writer text); card overlap re-mints re-argument. 050 edits exonerated (nothing they order targets crashed dimensions). Chorus discipline untested (no refrain family in draw). Floors 95/98%. No restore (smoking improved materially). Consecutive no-KEEP: 9. Drive complete — campaign summary written.
**Next direction:** Founder call for 051+. Candidates: ID-vs-silence precedence, CAPS-law relief (brief call), I-distribution (standing escalation), calibration queue (tight-shoes, header asymmetry, Carr-as-OUR). Never card-field edits, route-every-limit, diction pressure, factory-speech, willpower, re-argument. Do not replay 039/040.
