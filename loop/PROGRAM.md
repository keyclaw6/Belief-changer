...
7. `factory-learning-decide` uses a strict **non-promotional engineering transfer gate** over the bound held-out experiment: complete blinded/reversed judgments, one consistent independent judge instrument, no critical failures, no order instability, no secondary-dimension losses, and sufficient preregistered primary-dimension evidence in every held-out subject. It may retain a factory change without human release calibration, but it never makes the change release-eligible or establishes reader efficacy. Training-only improvement or held-out failure is not a factory win.
