...
Block adjacent cards that repeat the same principal encounter, break a
handoff, or reserve work backward. Block any card, adjacent or not, whose
job, resolved objection, belief now, encounter, or structure names a
justification, relapse door, or valuation that an earlier card's enacted
transition already corrected — including one door out of an earlier
card's totality — or that restages a debuted scene's full job (later
cards token-echo only). Name the pair (C-NN corrects it; C-MM re-opens
it) and the smallest correction: strike it from one card or move the
ownership. After the vow, block a
