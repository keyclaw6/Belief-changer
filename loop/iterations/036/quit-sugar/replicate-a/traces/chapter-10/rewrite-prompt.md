...
IN THIS CHAPTER — Friday calendar on the fridge, cliff edge in morning mist, empty box on the counter, cupboard bare but pockets full, tomorrow diary, bright sweetener packets, seatbelt on a quiet road
...
That is how the Sugar Trap holds after the best case has been answered. Not by force. By permission. One kept sweet keeps the whole belief lit, and the lit belief teaches the mouth that brightness closes hours.
...
One carries the binge because the craving, once triggered, carries the binge. That carrying is the trap signature, not your nature. It does not mean you are helpless for life. It means a lit wire hums. Take the belief out and the wire cools. Keep one exception and the wire stays warm.
...
The trap does not live in the cupboard. It lives in the reading that brightness closes hours. Move the paper and the reading walks with you. It finds new shelves. It learns new hours. It calls half past three an order and half past eight an order and cinema an order and tired an order.
...
The house rule did not cool the wire. It strung the wire across town.
...
Tomorrow lends today a glow it never owned. This last box must be enjoyed properly, so tasting strains, hurry grows, pictures brighten. The last week becomes a farewell tour, each dose watched too closely, each flood lent gold by ending. By the time tomorrow arrives the mouth has been taught for seven more days that paper is precious, and tomorrow needs another tomorrow to recover from the tour.
...
Tapering a wire that lives on pictures only brightens pictures. Small hits between gaps shout louder than plain days ever did. Each reduced dose is watched, weighed, praised for being small, mourned for being small, followed by counting to the next. The mind lives on the edge all week, measuring metres off a cliff.

You do not need to rehearse falling to learn ground. You need to see the cliff and step back.
...
That is what the body does with an accidental crumb. A sauce thickened without asking. A bite taken in polite half-light before the mind caught up. A recipe changed without warning. The body copes, steadies, drives on. The crumb revives nothing unless the reading is let back in.
...
You are one step from the last ordinary treat taken with attention, and you walk it with clear hands on an ordinary morning, free of paper and bargaining.
...
- A lighter packet or sweeter swap keeps the same reading that brightness closes hours, so the wire stays warm while paper changes.
...
- LENGTHEN to B±15% — Delivered 3035 words. Budget 4400. 0.85×B = 3740. Add ~705+ words serving the card job: "inside BAD SUGAR there is no safe cut-down, special occasion, tomorrow or substitute; totality is only stable state; the substitute idea itself is the flaw — swap the product, sweeteners included, and you remain a user, still hooked" — extend this card's own Friday-allowance week encounter, cliff, one / empty-house / tomorrow foreclosure, swap, and seatbelt evidence, not earlier cards.

- HEADER — Draft opens with workshop header: "IN THIS CHAPTER — Friday calendar on the fridge, cliff edge in morning mist, empty box on the counter..." Remove the IN THIS CHAPTER block entirely.

- UNASSIGNED-REFRAIN — Non-mantra phrase "brightness closes hours" recurs 3× verbatim: 1) "teaches the mouth that brightness closes hours", 2) "reading that brightness closes hours", 3) "reading that brightness closes hours, so the wire stays warm". Keep at most one occurrence, delete the repeats; do not invent a new mantra.
```
