...
the card reader, chapter-number ledger callbacks on cards, an evidence-ledger ID cited on more than one card (list every repeated ID with its cards; an evidence PASS must state that no ID repeats), and a structural-responsibility field that lists doors, scripts, doctrines, or thoughts instead of naming one room. Every ID a card cites must resolve against a plan-wide inventory
...
Your entire reply IS the review. End with exactly `fit to write from` or `needs changes first` as the contract requires.
