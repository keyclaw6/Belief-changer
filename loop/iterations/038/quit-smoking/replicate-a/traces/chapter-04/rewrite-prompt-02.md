...
```
REVISE
LENGTHEN to B±15% — Delivered 3071 words. Budget 4500. Required range 3825-5175. Add ~750-1500 words to reach B±15%.

Expansion assignment: Extend the undeveloped half of this card's job — creates the storm — at the same stress desk. The draft lands the single-minute failure that the dose leaves the papers untouched, but asserts the cumulative consequence in one line without enacting it. Quote the location to extend:

"It causes the aggravation. It does not relieve it."

Extend forward from that desk into the next stressful hour with the same row/bill/call still to face, showing in lived desk detail how using the dose as off-switch makes the next stress harder to meet — lost minutes, hurried thinking because the stick is burning, ash/smell/counting errands, packet-for-evening worry, and a second feed-call layered onto the next row. The extension must deliver new understanding: not just that one puff did not touch the row, but that repeated off-switch use compounds strain across a stressful day.

This target is not an earlier card's proof in a new setting and is not another proof of the already-landed single-minute conclusion; it is this card's own unspent consequence. Do not invoke C07 withdrawal-timing detail or C09 fear-of-life-without, and do not bring in C05 reward, C06 substitutes, or C10 patio material to fill words.
```
