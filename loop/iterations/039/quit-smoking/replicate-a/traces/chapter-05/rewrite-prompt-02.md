...
HEADER — draft opens with workshop header instead of earned prose. Quote:
`IN THIS CHAPTER / The empty corridor, the cooling paragraph, the step with the shopping bags, the filter held to the light`
Remove the inventory header. Enter the pellet-chain encounter directly in lived prose; do not replace it with a contents list.

LENGTHEN to B±15% — Delivered 3785 words. Budget 5000. 0.85×B = 4250, 1.15×B = 5750. Delivered is below range; add ~465+ words to reach 4250-5750.
Expansion assignment: extend the thin taste-without-paper encounter belonging to this card, quoted location:
`Take the coffee without paper. Hold it hot, smell it, drink it slow at the window. Does coffee lose its taste without paper? No. It gains it, because no stale overlay dulls the tongue.`
Extend what happens in the mouth across coffee and meal-end when the stale overlay is absent — lingering food taste, clean swallow, breath without sour cover, tongue ready for next taste — so the reader feels taste pleasure is fuller without paper, not merely intact. Check: taste-fullness is not done in C03 axis or C04 stress-kill; it does not repeat the already-landed corridor/paragraph/step work-credit proof in this draft, does not stage C10 patio social reward, does not argue C06 substitutes.
```
