...
The canonical judge route is two same-model replica identities executed as fresh native Codex subagents pinned to `gpt-5.6-sol` at `ultra`. That route was subsequently used for the identical-text control recorded above. V2 failed its frozen stability rule, so the degraded-reference and product invocations did not run. There is no external-access blocker; v2.1 is preregistered above and must be implemented, tested, and frozen before fresh controls.

## Judge-instrument baseline

Before any run-012 target-panel result, Stage-A judging changed from the legacy single chapter role to v2: belief-change efficacy and method integrity/epistemic safety judge the complete three-chapter block, while literary craft judges each chapter. Exactly two same-model native Sol-ultra replica identities covered every role target in both A/B orders. V2 failed because its collapse conflated absolute shared-label drift with comparative order instability. V2.1 retains the roles, prompts, schemas, identities, and matrix while correcting only that classification. Authorship detection remains absent.

This change answers an instrument failure, not H-049. V2.1 begins a new baseline and no resulting score is numerically comparable with v2 or legacy runs. H-049 was decided only by its frozen causal rule and the source-grounded specialist review; the panel cannot reverse or support that causal verdict. Explicit `--prompt` mode preserves the old runner solely for historical reproduction.
