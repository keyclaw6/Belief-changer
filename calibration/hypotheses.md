...
## H-040 — semantic gate then prose-only compression      [TESTING run-003]
...
- Rival explanations / signal risk: Improvement could come from receiving an extra call rather than role separation; run-002 R3 is the closest control because it used the same model, explicit length/rhythm instructions, and more semantic context yet expanded. A failure could reflect Opus reasoning-none constraint adherence rather than task interference.
- Next test: Run only the bounded compression pass on certified Chapter 2 R3 in run-003, then send its exact output to a fresh blind Sol reviewer. Do not change any shared prompt or plan unless this isolated test fails and its autopsy names a missing capability.
