...
- Next test: Hold the accepted R3 plan, style guide, writer prompt, Opus model, and reasoning mode fixed. Generate Chapters 2–3 with only the accepted prior chapter for continuity, then run objective and blind pairwise judgments. Attribute a downstream failure to H-039 only when missing or ambiguous context traces to information deliberately removed by normalization; otherwise open a separate chapter-writing hypothesis after the run.
