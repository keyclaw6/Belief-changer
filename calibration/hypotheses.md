...
## H-039 — normalized agentic master plan                [TESTING run-002]
...
- Result: Run-002 removed the diagnosed bookkeeping failure: its three reviews contained no occurrence-count, audit/state-matrix, repeated-SUMMARY, or cross-table-arithmetic blocker. Sol `ultra` produced a coherent 21-chapter, exactly 60,000-word plan at 11,512–11,712 words versus run-001's 16,405–19,166. R1 had two semantic defects (M-06 routing and no closing device). R2 fixed those but its whole-plan rewrite introduced a sucrose/sucralose typo; fresh review found an instruction/threshold contradiction and a closing image borrowed from the style guide. R3 resolved all known defects and received `fit to write from`. The plan therefore passed, but only on cycle 3: the at-most-one-substantive-revision prediction is falsified. Opus Chapter 1 R1 then received `REVISE` for M-02 capitalization, an internally inconsistent comparison, two unsupported expansions, omitted SC-08, missing ALL-CAPS landing, and prose metrics. Every required item was present in the accepted plan/style inputs; the reviewer identified no missing-context defect. Whole-artifact R2 used the same plan, prompt, style guide, model, and reasoning mode plus the R1 review. It corrected every blocker and received fresh `ACCEPT`; its reviewer found the assigned evidence, scenes, anatomy, mantras, and instruction complete and faithful. Chapter 2 R1 received `REVISE` for definition drift, premature LX-P03, wrong-chapter AN-10, omitted EV-17/SC-07, unsupported claims, banned register, and rhythm; every omission and boundary was explicit in the same inputs. R2 repaired that list but received `REVISE` for unscheduled pre-debut tokens, illicit repetition, omitted DEF-04, plan-ID leakage, unsupported universal language, and length/rhythm. Those boundaries were also explicit, so the observed failure remains writer execution rather than context deleted by normalization.
- Causal verdict: INCONCLUSIVE overall, with the bookkeeping submechanism supported and one positive downstream-sufficiency sample. Removing duplicated representations eliminated every run-001-style bookkeeping blocker, while the cycle-count prediction failed because independent semantic defects and one rewrite regression consumed all three rounds. Chapter 1 acceptance is evidence against context starvation, but Chapters 2–3 and blind product judgments are still required.
- Rival explanations / signal risk: A false positive would occur if Chapter 1's entry-contract card is sufficient while later writers or continuity seams need information absent from lean cards. A false negative would occur if independent writer defects are blamed on normalization merely because the plan is lean. Fresh-context reviews reduce reviewer-memory confounding; using the same planner model across plan rounds and one accepted chapter remain sampling/model confounds.
- Next test: Hold the accepted R3 plan, style guide, writer prompt, Opus model, and reasoning mode fixed. Run final whole-artifact Chapter 2 R3 from both accumulated reviews; if accepted, generate Chapter 3 with only the accepted prior chapter for continuity and run objective and blind pairwise judgments. Attribute a downstream failure to H-039 only when missing or ambiguous context traces to information deliberately removed by normalization; otherwise open a separate chapter-writing hypothesis after the run.
