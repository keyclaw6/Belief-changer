...
## H-041 — full versus minimal prose-editor context      [TESTING run-004]
...
- Rival explanations / signal risk: The same-pass A/B control removes the generic extra-call explanation. Provider sampling remains uncontrolled because each arm is one stochastic call; independent blind reviews and exact artifact preservation limit post-hoc selection.
- Next test: Preregister run-004, generate both arms from identical starting files, and give each exact output to a separate fresh Sol reviewer with multi-agent orchestration disabled but no time, token, or reasoning cap. Promote an artifact only on `ACCEPT`.
