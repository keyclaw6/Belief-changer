...
- Controlled production run: freeze the behavior-agnostic commissioner asset, generate all C-01-C-03 commissions in fresh Sol-ultra contexts, independently audit the whole set against the plan and exact packets, then freeze it before prose. Run unchanged Opus 4.6 reasoning-none writers and the ordinary review loop through all three chapters without a shared amendment; an earlier failure blocks promotion but does not truncate the causal sample.
- Prediction: all three grounded commissions are faithful and adequately concrete; all three first drafts have zero material evidence invention; all three chapters accept within the fixed cycles and the complete set passes existing length, craft, objective, and blind Stage-A gates.
- Decision rule: SUPPORTED only if the full prediction passes. REFUTED if adequate faithful commissions exist but at least two first drafts invent material evidence or any final chapter retains an invention. INCONCLUSIVE if a commission is materially unfaithful, the accepted evidence cannot support its assigned job, one isolated invention is repaired, or an independent craft/length/method dimension prevents full product passage.
