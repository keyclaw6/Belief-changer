...
- Result: Run-002 removed the diagnosed bookkeeping failure: its three reviews contained no occurrence-count, audit/state-matrix, repeated-SUMMARY, or cross-table-arithmetic blocker. Sol `ultra` produced a coherent 21-chapter, exactly 60,000-word plan at 11,512–11,712 words versus run-001's 16,405–19,166. R1 had two semantic defects (M-06 routing and no closing device). R2 fixed those but its whole-plan rewrite introduced a sucrose/sucralose typo; fresh review found an instruction/threshold contradiction and a closing image borrowed from the style guide. R3 resolved all known defects and received `fit to write from`. The plan therefore passed, but only on cycle 3: the at-most-one-substantive-revision prediction is falsified. Opus Chapter 1 R1 then received `REVISE` for M-02 capitalization, an internally inconsistent comparison, two unsupported expansions, omitted SC-08, missing ALL-CAPS landing, and prose metrics. Every required item was present in the accepted plan/style inputs; the reviewer identified no missing-context defect.
- Causal verdict: INCONCLUSIVE overall, with the bookkeeping submechanism supported and no current evidence of downstream context starvation. Removing duplicated representations eliminated every run-001-style bookkeeping blocker, while the cycle-count prediction failed because independent semantic defects and one rewrite regression consumed all three rounds. Chapter 1 R1's failures are writer execution failures on supplied context, not evidence that normalization deleted a required field. Acceptance and blind judgment across Chapters 1–3 are still needed.
- Rival explanations / signal risk: A false positive would occur if later writers or continuity seams need information absent from the lean cards despite Chapter 1 having enough. A false negative would occur if R1's six execution defects are blamed on normalization merely because the plan is lean. Fresh-context reviews reduce reviewer-memory confounding; using the same planner model across plan rounds and one Opus sample remain sampling/model confounds.
- Next test: Hold the accepted R3 plan, style guide, and writer prompt fixed. Regenerate Chapter 1 from its review, then generate and blindly judge Chapters 1–3. Attribute a downstream failure to H-039 only when missing or ambiguous context traces to information deliberately removed by normalization; otherwise open a separate chapter-writing hypothesis after the run.
