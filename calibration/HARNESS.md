...
- Judge gate (Stage-A v2.2, §9): both prompt controls pass; the role/identity/order matrix is complete; equal-role macro preference (ties = 0.5) ≥ **0.45**; every role ≥ **0.35**; zero unioned critical failures on ours. Comparatively order-unstable observations never contribute to a pass, and a role with no stable observation fails the gate. V2.2 has no authorship-detection probe.
...
| Product judges | **fresh native `gpt-5.6-sol` subagents, `ultra`** | exactly two same-model replica identities in Stage-A v2.2; never OpenRouter |
...
- **Stage-A v2.2 has three independent blind roles:** belief-change efficacy and method-integrity/epistemic safety each judge the complete three-chapter block; literary craft judges each chapter. Each returns strict role-specific JSON and one behavior-agnostic mechanism. No role guesses authorship or receives run history, hypotheses, amendments, source packets, or review output.
- **Independent-context replication rule:** exactly two native Sol-ultra replica identities judge every role target. Each identity/order judgment runs as a separately spawned fresh context and receives no other judge output. Replication comes from independent same-model contexts, not provider-family variation; it is not cross-family evidence. A missing identity, role target, or order invalidates the panel.
- Every replica identity covers every role target in both A/B orders. V2.2 retains v2.1 aggregation byte-for-byte in meaning: the comparative signature is product verdict, the winner sign for every score dimension, and critical labels unique to ours versus unique to reference. Absolute drift remains diagnostic, while collapsed `critical_ours` and `critical_ref` remain the union across both orders for the conservative zero-critical gate. With three chapters the frozen matrix is exactly 20 raw judgments and 10 collapsed identity observations.
- **Two preregistered v2.2 repairs:** native `--output-schema` must enforce the existing strict role schema without retry or repair; each role prompt must operationally define when every existing critical label applies and reserve it for a directly observable defect severe enough to make the text unsafe or unfit for that role. Labels, response fields, scores, comparative aggregation, and thresholds do not change.
- Before first product use, run both controls from scratch. Across the two fresh matrices, structured output must yield 40/40 strictly valid judgments with zero retries or repairs. The identical rule keeps its frozen thresholds. Degraded still requires the intact reference to win every collapsed observation with no v2.1 comparative-signature instability; the operational taxonomy must yield zero candidate-relative critical-label drift while retaining `incoherent_block_arc`, `broken_chapter_flow`, and `broken_continuity` for the corresponding grossly degraded role samples. Either control failure fails closed.
- **New-baseline discipline:** preserve v2 FAIL and v2.1 identical PASS/degraded FAIL exactly. V2.2 is a prospective non-comparable baseline with two independently scored repairs: syntax validity and critical-taxonomy reproducibility. Materials and degradation, score/control/product thresholds, native model/ultra route, two identities, fresh contexts, matrix, union safety, and v2.1 aggregation remain frozen. Use new directories; never reinterpret or overwrite prior evidence.
...
- Aggregates live in `judgments/judge-summary.json`. Product parity and causal movement are separate: the panel compares finished products but cannot establish why a candidate moved. Stage-A v2.2 is the prospective run-012 baseline after v2.1 failed its degraded control; its results are not numerically comparable with v2.1, v2, or legacy panels.
