...
- Judge gate (Stage-A v2.1, §9): both prompt controls pass; the role/identity/order matrix is complete; equal-role macro preference (ties = 0.5) ≥ **0.45**; every role ≥ **0.35**; zero unioned critical failures on ours. Comparatively order-unstable observations never contribute to a pass, and a role with no stable observation fails the gate. V2.1 has no authorship-detection probe.
...
| Product judges | **fresh native `gpt-5.6-sol` subagents, `ultra`** | exactly two same-model replica identities in Stage-A v2.1; never OpenRouter |
...
- Every replica identity covers every role target in both A/B orders. V2.1 defines one comparative signature per mapped raw judgment: product verdict, the winner sign for every score dimension, and critical labels unique to ours versus unique to reference. An observation is order-unstable only when those comparative signatures disagree. Absolute score shifts and critical labels shared by both candidates may vary across fresh contexts; record that drift diagnostically, never misclassify it as presentation bias. Collapsed `critical_ours` and `critical_ref` remain the union across both orders, preserving the conservative zero-critical safety gate. With three chapters the frozen matrix is exactly 20 raw judgments and 10 collapsed identity observations.
- Before first product use, run both controls from scratch. The identical rule keeps its frozen thresholds: complete matrix, tie in every collapsed observation, equal unioned critical sets, maximum within-call candidate score gap ≤1, and no comparative-signature instability. The degraded control still requires the intact reference to win every collapsed observation with no comparative-signature instability. Absolute drift remains reported but does not decide symmetry. Either control failure fails closed before product interpretation.
- **New-baseline discipline:** the committed v2 identical control remains FAIL under its frozen rule. V2.1 is one prospective instrument lever—comparative instability classification—and is not numerically comparable with v2 or legacy results. Judge prompts, score and gate thresholds, candidate/reference material, degradation, native transport, exact two identities, reasoning, and the 20-cell matrix remain frozen. Rerun both controls in new directories; never reinterpret or overwrite v2 evidence.
...
- Aggregates live in `judgments/judge-summary.json`. Product parity and causal movement are separate: the panel compares finished products but cannot establish why a candidate moved. Stage-A v2.1 is the prospective run-012 measurement baseline after v2 failed its first control; its results are not numerically comparable with v2 or the legacy panel.
