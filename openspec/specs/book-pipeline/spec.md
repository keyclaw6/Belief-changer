...
Chapters are written one at a time, in order, each by a fresh-context writer that sees only its authoritative inputs — the chapter's semantic authority (commission or plan card), the immediately previous chapter, and the style guide — then gated before advancing. Chapters SHALL be written one at a time, in order, by a fresh-context writer with that minimal context. Production chapters MUST be iterated to reviewer ACCEPT before publication or advancement. Calibration candidates governed by `loop/PROGRAM.md` use that runbook's judge gate on the campaign branch and are NOT published or advanced as accepted book chapters.
