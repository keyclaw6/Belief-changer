...
"$PYTHON" -m compileall -q scripts/bc_factory scripts/bc_autoresearch scripts/factory.py scripts/autoresearch.py scripts/publish_main.py scripts/eval/tests
"$PYTHON" -m unittest discover -s scripts/eval/tests -p 'test_*.py' -v
"$PYTHON" scripts/factory.py --help >/dev/null
"$PYTHON" scripts/autoresearch.py --help >/dev/null
