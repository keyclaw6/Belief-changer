...
from bc_factory.common import (FactoryError, atomic_bytes, atomic_json, canonical, confined, digest, exact_keys,
...
def instrument(repo: Path) -> dict:
    ...
    return {"schema_version": 2, "contract": contract, "dimensions": list(DIMENSIONS),
            ...
            "evaluation_code": {name: file_hash(repo / name) for name in ("scripts/bc_autoresearch/experiments.py", "scripts/bc_factory/schema.py")},
    ...
    for pair in spec["pairs"]:
        ...
        for order in ("AB", "BA"):
            ...
            if not path.exists():
                ...
                continue
            pair_task(repo, eid, pair["id"], order)
