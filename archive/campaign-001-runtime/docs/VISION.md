...
- **Whole-book iterations (revised 2026-09-05 — K1 dual-subject):** every loop iteration generates and judges one quit-sugar book and one quit-smoking book from the same factory files. Partial runs mislead; a one-subject swing is not a result. Same-subject replicate-b is optional later and is not the current KEEP object. Research artifacts are reused between iterations unless the hypothesis changes the research stage. Live campaign: `campaign-001`; next unit is `loop/CONTINUE.md`.
...
- **Evaluation stack (revised 2026-09-06):** (a) **reference-sighted judge panel** — chapter lanes (belief-mechanic, voice-emotion, reader-journey, chapter-comparison) plus book-arc and a Carr-distance 0–100 whole-book score; FAIL only on BLOCKING; KEEP reads census classes, comparison `missing`/`partial`, and Carr-distance `score-deficit` (`loop/judges/`, `docs/AUTO-TUNING-LOOP.md`). Census PASS is not Carr-likeness. Blind A/B panels are retired; (b) founder spot-reads as an optional anchor, never a required gate.
