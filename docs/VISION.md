...
- **Environments:** the repo is the state machine, so the loop runs anywhere. **Operator: the founder's GPT 5.6 Sole environment runs all calibration runs** (run-001 onward) from `calibration/HARNESS.md` — the harness was built 2026-07-10 (`calibration/` + `scripts/eval/`: operator manual, objective eval suite, blind judge protocols, run templates, hypothesis ledger). Hyperagent built and maintains the harness and is the escalation point. If a `gpt-5.6` alias is added to the LiteLLM proxy, GPT 5.6 is also available as writer and/or judge from any environment — the writer model is itself a tunable.
