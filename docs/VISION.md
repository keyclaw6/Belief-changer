...
- **Endless books** — one for every belief people want to change, produced by the agent pipeline (research → master plan → chapters) under a founder-held quality bar.
...
1. **Factory** — the book pipeline (research → master plan → chapters), a file-contract state machine in this repo; any competent agent environment can run it from files alone.
...
5. **Quality** — laddered evals (objective scripts → reference-sighted judge panels), outcome surveys once live, three learning tiers (book banks → style guide [founder-gated] → catalog).
...
- **Whole-book iterations (revised 2026-08-23 — two-book replicate):** every loop iteration generates and judges TWO independent whole quit-sugar books from the same plan (replicate A and B) — chapters change what they optimize across the arc, so partial runs mislead, and a single book confuses writer/judge noise with a real effect. Research artifacts are reused between iterations unless the hypothesis changes the research stage.
...
- **Evaluation stack (revised 2026-08-08):** (a) **reference-sighted judge panel** — three per-chapter lanes (belief-mechanic, voice-effect, reader-journey) plus a whole-book arc judge, each PASS-capable and evidence-quoting; the judges verify presence (assigned instruction/mantra verbatim, banned register, verbatim repetition) AND effect themselves, since there is no mechanical validator in the pipeline; comparing our chapters openly against the aligned real GSBS chapters (`loop/judges/`, `loop/reference-alignment.md`). Blind A/B panels are retired — we have the target, so the judges read both and name the gaps (`docs/AUTO-TUNING-LOOP.md`); (b) founder spot-reads as an optional anchor, never a required gate.
...
- **Environments (revised 2026-07-28):** the repo is the state machine, so the loop runs anywhere. **Operator: the founder's environment runs the loop** from `loop/PROGRAM.md` — the operator is plumbing; every measured model role is a spawned pi sub-agent — the auto-tuning loop (North Star: `docs/AUTO-TUNING-LOOP.md`) that replaced the 2026-07-12/13 rebuild after two external GPT 5.6 Pro reviews (2026-07-28): whole-book iterations, causal-cluster trace analysis, PASS-capable reference-sighted judges. Hyperagent built and maintains the loop and is the escalation point. **Model access is route-restricted per `loop/config.yaml` (sole authority):** Muse Spark writer/plan-writer/plan-reviewer/research/analyzer calls go through OpenCode Go as `muse-spark-1.3-contributor` (no completion cap); mid-chain Zen `muse-spark-1.3-contributor-free`; last fallback Vercel `meta/muse-spark-1.3-contributor` (same contributor weights; never the non-contributor alias). Cursor chapter judges stay Composer 2.5. On a Muse Spark route/quota/unavailable failure the executor retries that unit once on the fallback, then calls the founder if both routes fail (no hosted agent, no host system prompt underneath — the same rationale as Q1's standalone-harness decision). **Doctrines (founder, 2026-07-10):** (1) *prompts over determinism* — LLMs underestimate LLM intelligence, incl. their own; optimize with prompts and clever handoffs, deterministic code only for measurement/gates or after an autopsied prompt-level failure; (2) *research goes deep* — structured multi-subagent decomposition into recovery/experience communities (single-long-prompt is the baseline to beat; `prompts/research-agent.md` carries the depth doctrine and the 10× floors); (3) *failure autopsies required* — nothing REFUTED without an examined why; (4) *MLflow all-or-nothing* — if adopted, full depth (tracing, params/metrics/artifacts, prompt registry, comparison UI). **The operator executes the loop and has no authority over its design** (`loop/PROGRAM.md`, the judges, and the decision rules are frozen during a campaign; grievances go to `loop/learnings.md`) — the run-001–014 autopsy (2026-07-12) showed operator design-authority converts product time into process time. Founder gates remain on canon `main` and method integrity — now defined as **Carr-fidelity**: the factory matches Allen Carr's own practice (including his full-force scares, commands, and certainty) before any house twist is applied.
...
**Sequencing:** `production-books/quit-porn/` is **paused before planning** until Stage C passes; its research stays valid and it becomes the first novel-topic book of the proven factory.
