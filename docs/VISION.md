...
- **Environments:** the repo is the state machine, so the loop runs anywhere. **Operator: the founder's GPT 5.6 Sol environment runs all calibration runs** (run-001 onward) from `calibration/HARNESS.md` — the harness was built 2026-07-10 (`calibration/` + `scripts/eval/`: operator manual, objective eval suite, blind judge protocols, run templates, hypothesis ledger). Hyperagent built and maintains the harness and is the escalation point. **Model access: the founder's OpenRouter key**; writer arms to test (H-005): GPT 5.6 Sol, Opus 4.6 (reasoning none AND medium), Gemini 3.1 Pro, Moose Spark 1.1 when available — exact IDs resolved from the OpenRouter `/models` endpoint at runtime. **The operator has full authority over the calibration lab** (the harness manual, judge prompts, thresholds, tooling — under ledger/new-baseline discipline); founder gates remain only on canon `main` and method integrity.
