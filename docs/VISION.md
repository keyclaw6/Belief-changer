...
- **Whole-book iterations (revised 2026-07-28 — replaces the Stage A/B split):** every loop iteration generates and judges the WHOLE quit-sugar book — chapters change what they optimize across the arc, so partial runs mislead. Research artifacts are reused between iterations unless the hypothesis changes the research stage.
...
- **Evaluation stack (revised 2026-07-28):** (a) **mechanical validity gate** — `scripts/validate_chapter_anatomy.py`: anatomy presence, assigned instruction/mantra verbatim compliance, banned register, and the verbatim-repetition law, checked before any judging; (b) **reference-sighted judge panel** — three per-chapter lanes (belief-mechanic, voice-effect, reader-journey) plus a whole-book arc judge, each PASS-capable and evidence-quoting, comparing our chapters openly against the aligned real GSBS chapters (`loop/judges/`, `loop/reference-alignment.md`). Blind A/B panels are retired — we have the target, so the judges read both and name the gaps (`docs/AUTO-TUNING-LOOP.md`); (c) founder spot-reads as an optional anchor, never a required gate.
