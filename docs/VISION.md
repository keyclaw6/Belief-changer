...
- **Environments:** the repo is the state machine, so the loop runs anywhere. **Operator: the founder's GPT 5.6 Sol environment runs all calibration runs** (run-001 onward) from `calibration/HARNESS.md` — the harness was built 2026-07-10 (`calibration/` + `scripts/eval/`: operator manual, objective eval suite, blind judge protocols, run templates, hypothesis ledger). Hyperagent built and maintains the harness and is the escalation point. **Model access is route-restricted:** OpenRouter is used only for Opus 4.6 chapter writing and DeepSeek research; GPT/OpenAI, Gemini, Grok, planning, reviewing, auditing, and judging never use it. **Writer FIXED: Opus 4.6 with reasoning disabled, and Opus is chapter-writer-only** (founder correction 2026-07-10 — no writer-model experiments); the current product judges are fresh native GPT 5.6 Sol subagents at `ultra`, while other non-writer arms require an authorized native or direct route. **Doctrines (founder, 2026-07-10):** (1) *prompts over determinism* — LLMs underestimate LLM intelligence, incl. their own; optimize with prompts and clever handoffs, deterministic code only for measurement/gates or after an autopsied prompt-level failure; (2) *research goes deep* — structured multi-subagent decomposition into recovery/experience communities (single-long-prompt is the baseline to beat; OSS deep-research frameworks on the table — HARNESS §3b); (3) *failure autopsies required* — nothing REFUTED without an examined why; (4) *MLflow all-or-nothing* — if adopted, full depth (tracing, params/metrics/artifacts, prompt registry, comparison UI). **The operator has full authority over the calibration lab** (the harness manual, judge prompts, thresholds, tooling — under ledger/new-baseline discipline); founder gates remain only on canon `main` and method integrity.
