...
Factory-level learning is a separate OUTER loop and now has its own sealed runtime records. The learner proposes only generic holdout requirements; the independent reviewer must ACCEPT; `factory-learning-register` binds both outputs, actual model metadata and approved paths before edits; `factory-learning-freeze-change` validates the committed diff; only then may an independent selector reveal exact held-out topics; the transfer experiment is bound to those topics and factory hashes before `factory-learning-decide` can retain the change. Revealed topics are retired from the unseen pool for later cycles. Lower-level book agents remain book-focused.
