...
**Forbidden:** replacing that conversation with a process driver, Makefile, Temporal workflow, or a `run_factory.py`-style script that sequences the loop while the agent exits. Robustness belongs in the **harness** (pi 0.84.x: `pi-goal-x` with `/goal-direct` or `/sisyphus-direct` — continuation on `agent_settled`; Cursor: do not end the turn between units). Do not encode the continue-loop into factory prompts or skills. `npm:pi-agent-goal` is the same idea but peer-blocked on pi ≥0.81.
