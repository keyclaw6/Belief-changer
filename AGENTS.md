...
Provider-private reasoning state is not authoritative durable study state. If a same-model continuation fails specifically because historical reasoning `encrypted_content` was issued to a different provider caller, keep the same OpenCode session ID and controller model. Use the RDC provider-state recovery marker plus the project `provider-state-portability` outbound transform: it strips only pre-cutoff opaque reasoning identifiers from the in-memory provider request while leaving the stored transcript, visible/tool history, repository state and new post-cutoff reasoning untouched. Never repair this by switching models or rewriting the stored transcript.
