...
An explicit founder request to run the loop authorizes iterations. `loop/PROGRAM.md` is the sole operational runbook. `loop/config.yaml` is the sole model-route and parameter source. During a campaign, only files the runbook marks editable may change, inside the iteration's worktree. Judges, trace analysis, hypothesizing, plan review, writer, research, and planner all run as spawned pi sub-agents (`.pi/agents/*.md`) — the writer, plan-writer, and hypothesizer through the founder's Command Code proxy loopback (Muse Spark 1.2), the rest (research lead + sub-agents, plan-reviewer, judges, trace-analyzer) through the OpenCode Zen subscription (opencode, DeepSeek V4 Flash) — all per config. A role call carries only its role prompt and listed inputs: no hosted agent, no host system prompt. Calibration candidates produced by the loop use the runbook's judge gate and are not published or advanced as accepted book chapters.

## Dependencies
Understand or recreate: prefer dependencies fully reasoned about in-repo; reimplement small subsets over adopting frameworks. Model access goes through the founder's endpoints and always via environment variables; no provider keys belong in this repo. Route law (founder route change 2026-08-12): two routes, both exactly as configured in `loop/config.yaml`. The **Command Code** proxy loopback (`COMMANDCODE_API_KEY`, or the Command Code CLI login) carries ONLY the Muse Spark roles — the writer, plan-writer, and hypothesizer (Muse Spark 1.2 contributor, with the non-contributor variant as fallback; no `max_tokens`). The **OpenCode Zen subscription** (`OPENCODE_API_KEY`, the `opencode` route) carries every other role — research lead + sub-agents, plan-reviewer, judges, and trace-analyzer — on DeepSeek V4 Flash. There is no OpenAI subscription route. A role call carries only its role prompt and listed inputs: clean contexts, never a hosted agent with its own system prompt. On a route/credential failure the executor stops and calls the founder (the only documented fallback is the writer/plan-writer/hypothesizer contributor→non-contributor model).
