...
Cross-iteration optimization is now content-addressed rather than conversational. A completed baseline may receive one sealed learning packet that records protected strengths, improvement targets, recurring repairs and targeted research gaps; the packet is editorial/evaluation feedback and is never factual evidence. A successor prepared with `--learning-from BASELINE_RUN` freezes that exact packet into its manifest and supplies it to every role. Fresh runs must therefore preserve learned repairs instead of rediscovering or silently dropping them.

A completed successor that inherited learning is not allowed to become the next optimization baseline merely because its own final audit accepts it. Run the independent blinded AB/BA no-regression gate against its frozen baseline. Any consistent loss on argument, recognition, progression, voice, emotional movement or economy, or any critical candidate finding, yields `REPAIR_REQUIRED`; order-sensitive results yield `INCONCLUSIVE`. Only `PASS` may emit the next sealed learning packet through `advance-baseline`. A material regression may reopen only the whole-book editor in the SAME run, against the accepted assembly plus sealed regression feedback; the repaired assembly must be independently audited again and must receive a fresh AB/BA gate. This optimization baseline is separate from `factory/champion.json`: it cannot publish, claim efficacy, or replace the calibrated human-approved release gate.
