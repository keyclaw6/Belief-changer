...
When the host harness is OpenCode, never change its controller model as a recovery shortcut. Preserve the model recorded for that host job/session unless the owner explicitly asks for a model change. A provider/context failure should be diagnosed or compacted/retried on that configured model; do not silently substitute another model.

OpenCode-host recovery is harness-specific and not part of the Pi factory contract; provider-private reasoning state is not authoritative durable study state. If a same-model continuation fails specifically because historical reasoning `encrypted_content` was issued to a different provider caller, keep the same OpenCode session ID and controller model. The durable repair is store-level, not hook-level: the `provider-state-portability` outbound transform does not run before the provider rejects stale reasoning state, so do not rely on it alone. Run `scripts/opencode_provider_state_recover.py` (bounded idle-only backup, targeted sanitation of only pre-cutoff `metadata.openai` itemId/reasoningEncryptedContent, ID/count verification, audit entry) while leaving the stored transcript, visible/tool history, repository state and new post-cutoff reasoning untouched. Never repair this by switching models or rewriting the stored transcript.
...
Only the reusable BOOK FACTORY uses Pi as its agent runtime. `.pi/agents/` contains factory roles only: research, evidence review, planning, writing/review, state, whole-book editing, final audit, and the factory controller. The Pi `factory-orchestrator` is the factory controller; host harnesses may launch or supervise it, but they are not part of the portable factory runtime. Autoresearch roles live under `loop/` and must not be added as Pi wrappers.

`COMPLETE_UNRELEASED` is the factory handoff boundary. The factory creates and verifies one book, then returns control to its caller. Optional caller-supplied learning may be frozen into a run, but the factory never decides whether a book becomes the next baseline and never chooses its own future intervention.

Everything after factory completion belongs to the OUTER AUTORESEARCH loop: blinded AB/BA comparison, `REPAIR_REQUIRED / ADVANCE / PRESERVE_BASELINE` decisions, Factory Learner/Reviewer, held-out transfer tests, experiment interpretation, and selecting the next factory change. If outer evaluation finds a material regression, autoresearch may call the same factory back for the bounded whole-book repair already supported by the run; the evaluation itself remains outside Pi.

Factory-level learning must default to the smallest prompt/skill change that transfers. Do not add orchestration/state/code when the same correction can live cleanly in the existing factory contract. The long-term design goal is that the book factory can be lifted out of this repository without bringing the autoresearch loop with it.
