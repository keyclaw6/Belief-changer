# <Book Title / Working Title>

**Target behavior:** <e.g. gaming / doom-scrolling / porn / sugar>
**Status:** brief → researching → planning → drafting ch.N → done
**Created:** <YYYY-MM-DD>

A self-contained working folder for one Belief-Changer book. It fills up **in order**: brief → research → master plan (+ fresh allowed review) → chapters. Copy this `_template/` folder to `production-books/<book-slug>/` to start a new book.

## Artifacts (check off as created)
- [ ] `00-brief.md` — topic, reader, goal, fork decisions
- [ ] `research/research-log.md` — chronological log of every search
- [ ] `research/sources/` — raw source dumps (any format), one file per source
- [ ] `research/lived-experience.md` — synthesized from `sources/`: why they do it, why they can't stop, shame, testimonials
- [ ] `research/scientific-evidence.md` — synthesized from `sources/`: studies & mechanisms (supported vs contested)
- [ ] `master-plan.md` — chapter-by-chapter blueprint
- [ ] `master-plan-review.md` — fresh allowed reviewer critique + resolution (required)
- [ ] `chapters/` — `chapter-01.md`, `chapter-02.md`, …

## Canonical references (global — read, never copy in)
- **Style guide:** `/prompts/style-guide.md` (the writing bible)
- **Reference-book analyses:** `/analysis/` (where the style was extracted from)
