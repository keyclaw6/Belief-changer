...
If `caller_context` is present, treat it only as frozen caller-owned guidance: it is NOT empirical evidence. Use any `research_priorities` as questions to check against the current dossier; never let caller assertions substitute for current-subject support.
