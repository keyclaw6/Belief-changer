...
If frozen `caller_context.research_priorities` names a research gap, treat it only as a SEARCH PRIORITY for this subject. Re-establish the question from fresh current-subject sources. Caller guidance is not evidence: a phenomenon, mechanism, audience pattern or lived-experience theme from another context does not transfer by default.
...
Read `docs/RESEARCH-ACCESS.md`. Run `research-preflight --subject <slug> --live` (default bridge route over the working signed-in Chromium/OpenCLI setup; `--via cloak` selects the optional dedicated CloakBrowser profile with Agent-Reach at its pinned commit, loaded NopeCHA and its challenge check). Its doctor output is diagnostic only; actual general-web search/read, public Reddit search plus thread/comment reads, and authenticated X search/thread reads must succeed on the selected route. Reddit login state is diagnostic on the bridge route; missing required X login, expired session, failed read, blocked browser or exhausted solver quota means BLOCKED. Set up or refresh only the login required by the selected route, repeat preflight, and only then dispatch researchers. Never fabricate a READY report or send cookies/API keys to a model, Git, logs or chat.
