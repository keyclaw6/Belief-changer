...
If a sealed pre-research cross-iteration guidance object is supplied, the resulting research.json must also include a top-level `learning_context` object with exactly: `guidance_sha256`, `baseline_run`, and `gap_resolutions`. Copy the supplied guidance hash exactly. For every inherited research gap, include exactly one resolution with `gap`, `status` (`addressed | scoped_out | unresolved`), `evidence_ids`, and a nonempty `note`. `addressed` requires current-subject evidence IDs; prior-book material never counts as evidence. If status is `unresolved`, keep that exact gap in `open_questions`; if it is `addressed` or `scoped_out`, remove that exact gap from `open_questions` so it cannot be accidentally resurrected next iteration.
