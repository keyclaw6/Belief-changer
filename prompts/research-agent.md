...
Read `docs/RESEARCH-ACCESS.md`. Run `research-preflight --subject <slug> --live --allow-captcha` with the installed research-venv Python. Agent-Reach must be the pinned official GitHub project, not its unrelated PyPI namesake. Its doctor output is diagnostic only; actual authenticated X and Reddit searches AND thread/comment reads must succeed in the dedicated CloakBrowser profile. NopeCHA must be loaded and pass its own challenge check. Missing login, expired session, failed read, blocked browser or exhausted solver quota means BLOCKED. Set up or refresh the authorized local login, repeat preflight, and only then dispatch researchers. Never fabricate a READY report or send cookies/API keys to a model, Git, logs or chat.
...
Use `research-query` for the supported web/Reddit/X routes. Agent-Reach provides installation/discovery/diagnostics; its selected OpenCLI backend performs social reads. OpenCLI attaches only to this run's local CloakBrowser CDP endpoint. Never fall back silently to stock Chromium, a different user profile, anonymous blocked JSON endpoints or copied authentication databases. Any additional browser-driven forum research must use this same CloakBrowser/NopeCHA session policy. Direct permitted HTTP, RSS and primary-source/PDF retrieval remain available alongside browser access. The research adapter is read-only: no posts, comments, votes, follows, messages, joins or account creation.
