...
- **DEFAULT (Carr):** **Personify, exactly as Carr does.** Every book names its two mechanism characters — the trivial physical creature the reader will starve to death (the Little-Monster equivalent) and the belief-system that feeds it (the Big-Monster equivalent) — with no frozen proper names: refer to the two mechanisms only in plain everyday description as common nouns (the tiny physical craving and the brainwashing voice), never as capitalized branded creatures, never frozen on the mantra sheet, never invented Mite/Talker/Nipper-style tokens. The personification makes the craving *external, small, and winnable*; its "death throes" become something to revel in. It is never a mighty enemy the reader must battle with willpower — it is a starving parasite already dying. Burgeon's no-monster stance is a brief-level override, never a default.
...
- **Plain, confident, repetitive refrains.** Repeat key promises only by echoing their frozen mantra-sheet lines verbatim, exactly as frozen including endings; never build a chorus, pastoral refrain, or room-opening variant around them with changed endings.
...
4. **Only an assigned analogy or scene, never a quota.** Use a concrete analogy or scene only when the card assigns one for this chapter's job; never invent one to fill a chapter. Reuse the book's settled pictures and two creatures where the job allows; a chapter with no assigned analogy is not a defect.
...
### The chapter anatomy (typical Carr shape, not a mandatory identical template)
1. **"IN THIS CHAPTER"** — names of the rooms/pictures in this chapter, not a syllabus of what we will do, not "we will / you will," not a named pupil. Carr prints this; it is not factory-speech.
2. **Italic thesis line** — a spoken Carr sentence of the reframe, not a paste of the card's leaving-belief field.
3. **Body** — ONE belief-move, built through titled sections, landing on ALL-CAPS verdict lines. Plain conversational sentences. No fragment chains. No belief-free sensory catalogues.
...
**Chapter 1.** Paragraph one states the method promise flat: it will be easy, permanent, no willpower; the reader finds that hard to believe — read on. The scare this chapter owns lands at full force in the argument and is disowned in the same chapter. Do not route harm-lists to an appendix. Reviewer OVERCLAIM governs facts, not the method promise.
...
- **Practical-safety guardrail** (when the behavior touches medication/health): one plan-wide front-matter note (CA-01 split: clinical limits live here, never fused into instruction rows) routing medical specifics to a professional, kept outside the belief argument. Never paste the boxed workshop title mid-chapter. Never a mid-argument clinician disclaimer.
