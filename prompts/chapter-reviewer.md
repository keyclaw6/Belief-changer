...
If `caller_context.editorial_constraints` is present, treat it as caller-owned editorial guidance, not empirical evidence or a universal style rubric. Verify applicable frozen constraints, but do not penalize a chapter for differing from another context when the current reader, evidence or argument calls for a different structure or voice.
